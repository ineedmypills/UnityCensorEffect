    ...
    public class CensorEffect : MonoBehaviour
    {
        ...
        private void InitializeMaterials()
        {
            if (_censorMaskMaterial == null)
            {
                ...
                if (shader != null) _censorMaskMaterial = new Material(shader) { hideFlags = HideFlags.HideAndDontSave };
            }

            if (_censorEffectMaterial == null)
            {
                var shader = Shader.Find("Hidden/CensorEffect");
                if (shader != null) _censorEffectMaterial = new Material(shader) { hideFlags = HideFlags.HideAndDontSave };
            }
        ...
        private void CleanupMaterials()
        {
            ...
            if (_censorCamera != null) DestroyImmediate(_censorCamera.gameObject);
        ...
        private Camera GetCensorCamera()
        {
            ...
            {
                var go = new GameObject("Censor Mask Camera", typeof(Camera)) { hideFlags = HideFlags.HideAndDontSave };
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            var censorMaskTexture = RenderTexture.GetTemporary(source.width, source.height, 24, RenderTextureFormat.R8);
            ...
            if (CensorAreaExpansion > 0 && _blurMaterial != null)
            {
                ...
                var tempBlurTex = RenderTexture.GetTemporary(source.width, source.height, 24, RenderTextureFormat.R8);
